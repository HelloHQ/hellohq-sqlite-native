...
#
# The complete release build runs on Linux CI. A cr-sqlite-only local build can
# also run on macOS by setting HELLOHQ_ANDROID_CRSOLITE_ONLY=1; this is useful
# while validating a native producer fix before publishing a new release.
set -euo pipefail
...
CRSQLITE_ONLY="${HELLOHQ_ANDROID_CRSOLITE_ONLY:-0}"
if [ "${CRSQLITE_ONLY}" != "1" ]; then
  : "${OPENSSL_ANDROID:?set OPENSSL_ANDROID to a dir with <abi>/{include,lib} OpenSSL}"
fi
API="${ANDROID_API:-24}"
case "$(uname -s)" in
  Darwin) NDK_HOSTARCH=darwin-x86_64 ;;
  Linux)  NDK_HOSTARCH=linux-x86_64 ;;
...
while IFS= read -r so; do
  ...
  if [ "${CRSQLITE_ONLY}" = "1" ]; then
    find "${OUT}" -name 'libcrsqlite.so' | sort
  else
